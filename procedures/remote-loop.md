...
7. Trigger the review. **Do not fire if HEAD has not changed since the last trigger** (the runaway
   ...
   three ways on what the foreign trigger drew and where it is bound.**
   ...
   **And when it drew one that HEAD has already moved past, the re-take happens in this run with no
   ...
   commit in hand, so the foreign baseline is spent and the trigger this step fires asks about a
   commit nobody has asked about. What it costs is the ancestor review's findings, discarded unread
   and named in the report — which is the ruling step 9 already makes for an ancestor on this loop's
   own baseline.
   ...
   **None of the five is a licence to fire again on a trigger that was answered with a review**,
   ...
   re-takes is one whose review cannot bind the commit in hand at all — so in both cases the trigger
   fires on a question the reviewer has not been asked, which is the distinction the whole
   enumeration turns on.
...
9. Decide continue / finish / abort in one line — **with one qualification the two clean rows carry
   ...
   **ancestor-relaxed selection** is the selection above with exactly one condition changed:
   ...
   unchanged, so what it widens is the commit and nothing else. **Non-empty means the configured
   reviewer answered the hand-typed trigger and the branch has since moved past that answer**: take
   the `foreign-baseline-retake` row. Empty means there is nothing of the reviewer's behind this
   ...
   **What licenses the trigger is that nothing standing can bind a verdict to HEAD.** The review is
   ...
   read as an answer about the commit in hand, by this loop or by anyone. The re-take therefore fires
   at a commit **no trigger has ever asked about**, which is the adoption row's own sentence and the
   reason the racing-a-person argument survives the widening. It is not a race for the newest comment
   on a commit somebody is still discussing — that commit has been superseded, and in the case this
   row is written for it was superseded by a push this run's own predecessor made.
   ...
   **What cannot be established about an `EXTRA=` here is which trigger it answers, not who wrote
   ...
   | ---------------------------------------------------------------------------------------- | --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | `marker_head=none` + the selection above is non-empty                                    | **adopt** (`foreign-baseline-adopt`)    | A hand-typed trigger took the baseline and the reviewer answered it **at the commit in hand**. **Any verdict form opens this** — `review`, `comment` or `reaction` — because the selection reads the review list and takes only `trigger=` off the line. Go to 10 and read **every review in the selection** — **this opens no round, posts nothing, and does not spend `--max-rounds`**, so it is a read and not a re-take. The binding is GitHub's own `commit_id`, read from the review list and compared in full, which is stronger than the marker binding the row below was written to protect. **The review adopted need not be the one the fence named**: with `bot=` empty that line carries the newest review by any bot. Step 11 says what the replies are scoped by. Then **go back to step 7 for the ordinary re-take** — an adopted round never converges the loop and never merges. **Say in the report which of the two states this fired from**, naming **every** adopted `review_id=` and, when the fence's primary line was not one of them, the review it named instead; and append one line to `.revloop/field-notes.md`: a baseline a hand-typed trigger took is adopted **under a foreign baseline**, and this run's own trigger with a marker the fence could not read is adopted **over a garbled marker** — step 7 gives both spellings and they are not interchangeable |
   | `marker_head=none` + the selection is empty + the **ancestor-relaxed** selection is not  | **re-take** (`foreign-baseline-retake`) | The reviewer answered the hand-typed trigger and the branch has moved past that answer, so nothing standing can bind a verdict to HEAD. **Read nothing and reply to nothing**: go back to step 7 and open a **new** round with an **ordinary** trigger — no `attempt=`, the round advances, `head=` and `oid=` are the current HEAD — subject to `--max-rounds` as any round is. The ancestor review's findings are **discarded unread**, which is the ruling the ancestor row below already makes for this loop's own baseline. **Name the discarded `review_id=` in the report and say its findings were not read**, print whether this loop's `revloop:reply` markers already sit under it — a value nothing here decides on — and append one line to `.revloop/field-notes.md`. **This is the interrupted adoption's only way home**: the fixes are pushed, HEAD has moved, and the review that licensed them no longer matches HEAD                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
10. Read the findings — **from the inline comments and from the review body, because either can
    ...
    **An adopted round reads the selection step 9 adopted, instead of the `review_id=` on its
    line** — which is more than one review whenever the hand-typed trigger drew more than one, and
...
- **Never re-fire the trigger without new commits — unless nothing of yours can still bind a verdict.**
  ...
  **no verdict at all** is, so far as this run can tell, a comment that went nowhere, and refusing to
  ...
  the run that met the reply; the lost-baseline one is, whenever step 9 could rule on the review the
  foreign trigger drew** — adopting it at the commit in hand, or discarding it unread when HEAD has
  moved past it. Without either ruling it too waits for a later run. **Only the first two of the
  ...
  **The premise is what the invariant
  ...
  run — the person's request has been answered and read, so nothing is being raced. **The fourth is
...
- **Step 9's `foreign-baseline-adopt` row.** The failure it answers is measured three times — a
  ...
  half of this row with no measurement at all.
...
- **Step 9's `foreign-baseline-retake` row.** The failure it answers is derived rather than
  ...
  none has printed whether this loop's replies sit under it — the value this row deliberately does
  not decide on.
