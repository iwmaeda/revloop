...
9. Decide continue / finish / abort in one line — **with one qualification the two clean rows carry
   ...
   **Only a `review` may be adopted, and only a `review` line opens the selection.** A `comment`
   carries no commit binding at all, which is `docs/design-notes.md`'s too-old direction and the one
   that ends in a false clean; a `reaction` carries neither `login=` nor `commit=`; a `pending`
   carries no marker fields, which is why step 7 refuses to let one establish a foreign baseline in
   the first place. **Opening the selection on the `comment` and `reaction` lines as well was
   considered and declined, and the reason is arithmetic rather than taste.** The fence prints a
   `comment` only when its review selection came back empty and a `reaction` only when both did, and
   under `marker_head=none` its bot filter admits everyone — so on those two lines there is no review
   strictly after the trigger to find, by anybody, and the `reviews(last:15)` window cannot be hiding
   one, because a review that fell out of that window needs fifteen newer reviews and every one of
   them is newer than the trigger too. The only review a wider gate could reach is one sharing the
   trigger's own second, which the lower-bound paragraph above declines on its own terms.
...
- **Step 9's `foreign-baseline-adopt` row.** The failure it answers is measured three times — a
  ...
  after the trigger that took the baseline, and it can neither converge the loop nor merge. **The
  fixtures cannot pin which of the two `marker_head=none` rows step 9 takes**, and the reason is now
