...
9. Decide continue / finish / abort in one line — **with one qualification the two clean rows carry
   ...
   **Three conditions, all of them, and each one is somebody's abort if it fails.** The line is a
   `review` carrying `marker_head=none`; `login=` equal to the resolved reviewer with a trailing
   `[bot]` stripped from **both** sides; and the **fetched** `commit_id` from check (e) equal to
   `git rev-parse HEAD` in all forty characters. The login condition is not a formality here — on a
   ...
   **The review's `state` is deliberately not a fourth, because this step cannot read one.** The
   fence filters on `state != "DISMISSED"` inside itself and then **drops the state from its output**
   — step 10 says so in as many words — so a `review` line carries `at=`, `login=`, `review_id=` and
   `commit=` and no state at all. A condition listed here would be one nothing on the line can answer,
   and the likelier reading of an unanswerable condition is that it was satisfied because nothing
   contradicted it. **The check is step 10's and it is already there**: that step enumerates the state
   before reading anything else and fails closed on one its table does not list, which an adopted
   round reaches by the same edge every other round does. This paragraph exists because the condition
   was listed here first.
   ...
   **Only a `review` may be adopted.** A `comment` carries no commit binding at all, which is
   ...
   `EXTRA=` alongside an adopted review aborts nothing**, and the reason is not the one 0.10.0 gave
   for its own rows. That reason — the trigger is by definition not one this run posted — is **false
   here in one case**: a focus carrying the literal `revloop:trigger` garbles this run's **own**
   marker, and the adoption row can fire on it, so the trigger is this run's after all. **The ruling
   survives on two reasons that do not depend on whose trigger it was.** The `EXTRA=`'s author is
   unverifiable under any `marker_head=none`, for the reason the next paragraph gives. And **an
   adopted round has the configured reviewer's review of this very commit in hand** — a reviewer that
   answered is a reviewer that was not out of quota when it answered, so a notice beside that review
   is stale, or somebody else's, and in neither case is it this round's verdict.
   ...
   **any** bot — measured: an adoptable primary line by the configured reviewer arrives beside an
   ...
   | ------------------------------------------------------------------------------------------ | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
   | `review` + `marker_head=none` + `login=` is the reviewer + fetched `commit_id` equals HEAD | **adopt** (`foreign-baseline-adopt`) | A hand-typed trigger took the baseline and the reviewer answered it **at the commit in hand**. Go to 10 and read it — **this opens no round, posts nothing, and does not spend `--max-rounds`**, so it is a read and not a re-take. The binding is GitHub's own `commit_id`, fetched by `review_id=` and compared in full, which is stronger than the marker binding the row below was written to protect. Reply under `round=adopted-<review_id>`, then **go back to step 7 for the ordinary re-take** — an adopted round never converges the loop and never merges. **Say in the report which of the two states this fired from**, naming the `review_id=` either way, and append one line to `.revloop/field-notes.md`: a baseline a hand-typed trigger took is adopted **under a foreign baseline**, and this run's own trigger with a marker the fence could not read is adopted **over a garbled marker** — step 7 gives both spellings and they are not interchangeable |
