...
| フェーズ     | ステップ | 内容                                                                                                                                                            | 目安の時間          |
| ------------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| **解決**     | 1        | リポジトリを調べ、`source` 列付きの設定表を表示する。                                                                                                           | 数秒                |
| **準備**     | 2–6      | topic branch を切り、検証コマンドを実行、push する変更に input-space と definition の掃きをかけ、コミット分割を提案し (**1 つ目の停止点**)、push して PR を作る | 3分                 |
| **トリガー** | 7        | レビュー依頼のコメントを投稿する。`revloop:trigger` マーカーにレビュアー・bot・head の sha・ラウンドを記録する                                                  | 数秒                |
| **待機**     | 8        | **今回の**トリガーに対する verdict が現れるまで GitHub をポーリングする                                                                                         | **数分 — 下記参照** |
| **判定**     | 9        | continue / finish / abort を判定する                                                                                                                            | 数秒                |
| **修正**     | 10–11    | inline の指摘を読んで修正し、そのクラスに合う掃き方を選んで掃き、すべての指摘に返信する                                                                         | 数分                |
| **完了**     | 12       | 報告する。`--merge` が設定されている場合、 CI の green を待ってからマージする (**2 つ目の停止点**)                                                              | CI 次第             |
