...
both abort on a match **without retrying inside the run that hit it**: the quota recovers with time,
and another round spends the loop against a reviewer that cannot answer. **The recovery in both loops
is a later invocation** — the local loop's runaway invariant is a within-run rule, so a fresh session
simply reviews again, while the pull-request loop re-takes a standing rate-limited trigger with a new
round when the run reading the reply is not the run that posted it, bounded by `maxRounds`.
Match the fixed part of the message and never a reset time
or a count, which differ every round. A pattern that is too loose **fails closed in both loops, by two
different orderings**. Locally it can only turn one abort into another, because the row that reads it
sits above the other aborts and far above the clean row. On the pull-request loop the clean-comment
row sits **above** the two rate-limit rows, so a loose pattern cannot reach a clean finish there
either; what it can reach is an abort-class bot comment, which it turns into a re-take that spends one
round. Neither ordering lets a run finish clean over a review that did not happen. One that is too
tight leaves the reply falling to `unparsed-review-output`, which is
